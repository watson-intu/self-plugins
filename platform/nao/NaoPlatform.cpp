...
NaoPlatform::NaoPlatform( const std::string & a_URL /*= ""*/ ) :
	m_spSession( new qi::Session() ), 
	m_URL( a_URL ),
	m_bExtraMovementDisabled( false ),
	m_bAutonomousLifeStarted( false ), 
	m_ExtraMovementDisableInterval( 60.0f )
{
	sm_pInstance = this;
	...
	if ( m_URL.size() == 0 )
	{
		...
		if ( pInstance != NULL )
			m_URL = pInstance->GetLocalConfig().m_RobotUrl;
		else
			m_URL = "tcp://127.0.0.1";
	}
...
void NaoPlatform::SetCurrentVolume( float a_fCurrentVolume )
{
	m_VolumeNotifierList.Invoke( a_fCurrentVolume );
}

void NaoPlatform::AddVolumeNotifier( Delegate<float> a_Delegate )
{
	m_VolumeNotifierList.Add( a_Delegate );
}

bool NaoPlatform::RemoveVolumeNotifier( void * a_pObject )
{
	return m_VolumeNotifierList.Remove( a_pObject );
}