...
bool NaoHealthSensor::OnStart()
{
	...
	OnPostureChanged("Stand");
	
	m_VolumeNotifier = DELEGATE(NaoHealthSensor, OnVolumeChanged, float, this);
	pPlatform->AddVolumeNotifier(m_VolumeNotifier);
...
void NaoHealthSensor::OnVolumeChanged( float volume )
{
	BuildHealthData("volume", "", "", volume, false, false);
}
