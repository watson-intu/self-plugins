...
REG_OVERRIDE_SERIALIZABLE(SoundGesture, WinSoundGesture);
REG_SERIALIZABLE(WinSoundGesture);
RTTI_IMPL(WinSoundGesture, SoundGesture);

bool WinSoundGesture::Execute(GestureDelegate a_Callback, const ParamsMap & a_Params)
{
...
bool WinSoundGesture::Abort()
{
	return false;
}

void WinSoundGesture::Play()
{
	...
	if (pReq != NULL)
	{
		...
		ThreadPool::Instance()->InvokeOnThread(VOID_DELEGATE(WinSoundGesture, PlayThread, this));
	}
}

void WinSoundGesture::PlayThread()
{
	Request * pReq = ActiveRequest();

	WinSpeechGesture::InitDS();
	...
	if (bWebRequest)
	{
		...
		IWebClient::SP spClient = IWebClient::Create(m_Sound);
		spClient->SetStateReceiver(DELEGATE(WinSoundGesture, OnStreamState, IWebClient *, this));
		spClient->SetDataReceiver(DELEGATE(WinSoundGesture, OnStreamData, IWebClient::RequestData *, this));
	...
	else
	{
		...
		if (sound.LoadFromFile( file ))
		{
			...
			// Create buffer. 
			HRESULT hr = WinSpeechGesture::sm_pDirectSound->CreateSoundBuffer(&dsbdesc, &m_pBuffer, NULL);
			if (SUCCEEDED(hr))
			{
		...
		{
			Log::Error("DSSoundGesture", "Failed to load WAV: %s", file.c_str() );
		}
	}

	ThreadPool::Instance()->InvokeOnMain(VOID_DELEGATE(WinSoundGesture, OnPlayDone, this));
}

void WinSoundGesture::OnPlayDone()
{
	m_pBuffer.Release();
...
void WinSoundGesture::OnStreamState( IWebClient * a_pClient )
{
...
void WinSoundGesture::OnStreamData( IWebClient::RequestData * a_pData )
{
